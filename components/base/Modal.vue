  ...
  <div class="modal">
    <div class="modal__dialog">
      ...
      <div class="modal__footer">
        <slot name="footer">
          <button v-if="closeText" type="button" class="cancel-btn" @click="close">
            {{ closeText }}
          </button>
          <button v-if="actionText" type="button" class="action-btn" @click="$emit('action')">
            {{ actionText }}
          </button>
        </slot>
      </div>
...
export default {
  ...
  props: {
    type: {
      ...
      default: 'general',
      validator (value) {
        return ['general', 'confirm'].includes(value)
      }
    },
    title: {
      type: String,
  ...
  emits: ['close', 'hidden', 'shown', 'action'],
  data () {
    return {
      showing: false
    }
